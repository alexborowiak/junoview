...
def render_item(item: Item, sec_id: str = "") -> str:
    ...
    return (
        ...
        f'{id_tag}{trace_btn}'
        # Pin means the filters cannot reach a cell. Labels are deliberately
        # separate and multi-select: reactions and categories belong on the
        # cell without moving it away from its existing outline row.
        ...
        f'title="Label this cell with reactions or categories" '
        f'aria-label="Label this cell">{_ic("tag")}</button>'
...
def render_sections(doc: Document) -> str:
    ...
    for s in doc.sections:
        ...
        sections_html.append(
            ...
            f'&#9662;</span><span>Collapse</span></button>'
            f'<div class="sectionhead-txt" data-sec="{sid}" role="button" '
