...
ANNOT_COMMON: dict[str, tuple[type, str]] = {
    ...
    "anim": (dict, "How this item arrives during playback. `type` names "
                   ...
                   "click."),
    "out": (int, "The build ORDER on which this object GOES AWAY again. "
                 "Absent -- as in every deck written before it existed "
                 "-- means it stays. A PEER of `anim` rather than part "
                 "of it, because the commonest swap of all is \"this "
                 "picture is simply there, and that one replaces it on "
                 "the first click\", and an object that is simply there "
                 "has no `anim` to hang an exit off. An ORDER rather "
                 "than a stop, so a swap does not drift apart the "
                 "moment a flip book is added earlier on the slide. It "
                 "must be later than this object's own `anim.order` "
                 "when it has one, and one naming a build that no "
                 "longer exists is ignored, leaving the object on the "
                 "slide -- every unresolvable reference on a slide "
                 "fails open, so nothing ever vanishes for good."),
