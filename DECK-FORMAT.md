...
| `video` | `x`, `y` | A video or audio clip. `vkey` names its bytes in the deck's `media` store; `poster` is the still shown before it plays; `trim` `{s, e}` the seconds it plays between; `audio` 1 for a sound-only clip; `ctrl`, `auto`, `loop`, `mute` are its playback switches. It exports as a real media shape in a `.pptx`. |
| `rect` | `x`, `y` | A drawn shape; `shape` picks which one: absent is a rectangle, else `ellipse`, `triangle`, `diamond`, `pentagon`, `hexagon`, `star`, `cross`, `arrow`, `heart`, `cloud`, `bubble`, `lightning`, `exclaim`, `question`, or an open stroke that takes no fill — `langle`, `rangle`, `lbrace`, `rbrace`, `lbracket`, `rbracket` (T419). |
| `web` | `x`, `y` | A live web page: `url` (http or https only) is drawn as a sandboxed iframe — live in playback and in an exported page, covered by a label while editing so the box can be moved. Only the address is stored. A `.pptx` has no shape for it; the export says so. |
| `table` | `x`, `y` | Rows of plain strings, not HTML. A column may say what it **holds** in `ctype[i]` — `t` (`'num'`/`'text'`; absent means read it from the cells), `dp` decimals, `thou` a thousands separator, `pre`/`suf` around each number, `align` — and its alignment, its decimals and its units follow from that. `calc[i]` names a footer function (`sum`, `mean`, `min`, `max`, `count`), computed at draw time and never stored; `rules[i]` is its conditional format (`{kind: 'scale'|'above'|'below', at, color}`); `groups` is the spanning row above the header, a list of `{at, n, text}`. All of it exports as a real `.pptx` table, merged header included. |
| `chart` | `x`, `y` | A native chart: `ct` (bar/line/scatter/pie), `cats`, and `series` [{name, ys, color}] carry the numbers; `ref` links it to the table card it was born from. A series may add `axis` (`'y2'`, read against a right-hand axis), `trend` (`'linear'`), `ct` (`'line'`, drawn over the bars), `hide`, `err` (one error per point) and `band` (`{lo, hi}`); the chart may add `stack` (1 or `'pct'`), `ylog`, `labels`, and the axis titles `xlab`, `ylab`, `y2lab`. It exports as a real `.pptx` chart, switches included (a band has no `.pptx` twin). |
...
| --- | --- | --- |
| `hide` | int | 1 when this is hidden: not drawn while editing, in playback, in print or in an exported `.pptx`, and it claims no click. A spare kept on the slide. |
...
| `anim` | How and when this item arrives: `{type, order}` plus the optional `by` (`para`/`sent`, for a text box that arrives a bullet or a sentence at a time), `hl` (1 to show every piece from the start and light up the piece each click is about instead of hiding the rest), `sync` (the `fid` of a flip book on the same slide whose figure k shows with piece k, on the same click; the book then has no clicks of its own — T418) and `after` below. `type` is one of `appear`, `fade`, `rise`, `zoom`, `slide` (fly in), `turn` (page turn) or `type` (typewriter, words only). |
| `win` | dict | Pictures only: a window onto the picture, `{x, y, w, h}` in percent of the picture, shown filling the box — the picture is scaled so that part fills it. A zoom callout is a picture wearing a window; the picture itself is never altered. |
| `sync` | int | Pictures and figures that zoom together: every item on the slide with the same number is enlarged alongside this one when any of them is enlarged in playback (Z, Alt+click, tap). |
| `motion` | str | A movement that keeps going while the object is on the slide: `wobble`, `bob` (float), `pulse`, `sway`, `shake`, `spin`, `swing`, `breathe`, `jelly`, `drift`, `glow`, `tilt` or `flicker`. Absent means still. |
| `mo` | dict | The numbers on that movement (T445), each absent when it is the movement's own: `sp` speed as a multiple of its own pace, `amp` how far as a multiple of its own travel, `ease` a CSS easing name, `dl` seconds it waits after arriving before it starts, `n` how many cycles it runs (0 is for ever) and `dir` `-1` to run the cycle backwards. The keyframes multiply their travel by `amp`; the rest are the `animation` shorthand's own terms, written onto the element by `motionPaint`. |
