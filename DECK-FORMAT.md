...
| `anim` | How and when this item arrives: `{type, order}` plus the optional `by` (`para`/`sent`, for a text box that arrives a bullet or a sentence at a time), `hl` (1 to show every piece from the start and light up the piece each click is about instead of hiding the rest) and `after` below. `type` is one of `appear`, `fade`, `rise`, `zoom`, `slide` (fly in), `turn` (page turn) or `type` (typewriter, words only). |
