...
| `web` | `x`, `y` | A live web page: `url` (http or https only) is drawn as a sandboxed iframe — live in playback and in an exported page, covered by a label while editing so the box can be moved. Only the address is stored. PowerPoint has no shape for it; the export says so. |
| `table` | `x`, `y` | Rows of plain strings, not HTML. A column may say what it **holds** in `ctype[i]` — `t` (`'num'`/`'text'`; absent means read it from the cells), `dp` decimals, `thou` a thousands separator, `pre`/`suf` around each number, `align` — and its alignment, its decimals and its units follow from that. `calc[i]` names a footer function (`sum`, `mean`, `min`, `max`, `count`), computed at draw time and never stored; `rules[i]` is its conditional format (`{kind: 'scale'|'above'|'below', at, color}`); `groups` is the spanning row above the header, a list of `{at, n, text}`. All of it exports as a real PowerPoint table, merged header included. |
