...
.pb-collapse{font-family:var(--mono);font-size:12px;border:1px solid #ffffff22;
  background:#ffffff0a;color:#cfe0ea;width:30px;height:30px;flex:none;
  border-radius:var(--rad);cursor:pointer;padding:0;line-height:1;
  box-sizing:border-box;}
.pb-collapse:hover{color:#fff;border-color:var(--cyan);}
body.pbpos-right .pb-collapse{width:100%;}
/* folded away: the small handle left over, on the edge it folded into */
.present-bar-show{position:fixed;z-index:141;
  font-family:var(--mono);font-size:11px;letter-spacing:.04em;
  border:1px solid #ffffff26;background:var(--chrome);color:#cfe0ea;
  cursor:pointer;height:30px;padding:0 12px;display:none;
  align-items:center;gap:7px;box-shadow:0 6px 22px #00000055;}
.present-bar-show[hidden]{display:none!important;}
body.doc-presenting.pb-folded .present-bar-show{display:inline-flex;}
.present-bar-show:hover{color:#fff;border-color:var(--cyan);}
/* top: the handle hangs from the top edge; right: it tucks to the side */
body.pbpos-top .present-bar-show{top:0;right:18px;border-top:none;
  border-radius:0 0 var(--rad) var(--rad);}
body.pbpos-right .present-bar-show{top:14px;right:0;border-right:none;
...
(function(){
  ...
  function buildTree(sh){
    ...
    toolBtn('−',function(){treeZoomBy(host,1/1.2);})
      .title='Zoom out';
    var zoomLbl=toolBtn('100%',function(){treeZoomSet(host,1);});
    zoomLbl.classList.add('tt-zoomval');
    zoomLbl.title='Zoom — click to reset';
    toolBtn('+',function(){treeZoomBy(host,1.2);}).title='Zoom in';
  ...
  (function(){
    ...
    function applyPbDock(){
      ...
      var cb=$('#pb-collapse');
      if(cb){
        cb.innerHTML=(pbDock==='top')?'&#9650;':'&#9654;';
        cb.title=(pbDock==='top')
          ?'Fold the bar up out of the way'
          :'Fold the bar off to the right';
      }
      var sh=$('#present-bar-show');
      if(sh) sh.innerHTML=(pbDock==='top')
        ?'&#9776; Controls':'&#9776;';
...
<div class="present-bar" id="present-bar" hidden>
 ...
 closed or lost.">&#10005; Exit presentation</button>
  ...
  <button class="pb-collapse" id="pb-collapse"
    title="Fold these controls out of the way">&#9650;</button>
</div>
...
def _self_test() -> None:
    ...
    assert ".appbar .toggle.sub{height:22px" in out
