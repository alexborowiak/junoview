...
`_as_presentations` rebuilds each deck field-by-field, so a key it does not
...
of them through `_as_presentations`, and demands every one survives. The
...
def test_deck_level_keys_normpres_keeps_survive_the_python_rebuild():
    ...
    assert not unknown, (
        ...
        "_as_presentations (presentations.py), or the key will silently "
        "die on every project save.")
    deck = {"name": "parity", "slides": [], **DECK_SENTINELS}
    out = _as_presentations([deck])
    assert len(out) == 1
    entry = out[0]
    dropped = sorted(k for k in kept if k not in entry)
    assert not dropped, (
        f"_as_presentations dropped deck-level key(s) {dropped} that "
...
def test_slide_level_keys_normpres_keeps_survive_the_python_rebuild():
    ...
    assert not unknown, (
        ...
        "slide rebuild in _as_presentations (presentations.py).")
    ...
    assert not dropped, (
        f"_as_presentations dropped slide-level key(s) {dropped} that "
